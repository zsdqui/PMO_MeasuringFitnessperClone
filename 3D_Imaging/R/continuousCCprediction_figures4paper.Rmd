...
Figure 2A: Nuclei, mitochondria and cytoplasm of 10 representative cells from region highlighted in (A). Every data point is color-coded according to its cell membership. 
```{r}
coi=as.character(c(149, 222, 25, 259, 237, 45, 139, 82, 26, 89, 21, 215))
coi=setdiff(coi, rownames(imgStats_)[imgStats_$segmentationError])
coord__=coord_[coord_$id %in% coi,]
  
figname=paste0(OUTFIGS, filesep, "Fig2A_snapshot.tif")
if(!file.exists(figname)){
  ...
  q <- cbind(id = unique(coord__$id)[order(unique(coord__$id))], cell = c(1:length(unique(coord__$id))))
  q <- as_tibble(q)
  rgl::legend3d("bottomright", as.character(q$cell), fill=col[as.character(q$id)])
...
png(figname)
im <- bioimagetools::readTIF(paste0(OUTFIGS, filesep,"Fig2A_snapshot.tif"))
im <- EBImage::resize(im, dim(im)[1]/1)
plot(raster::as.raster(im[,,,1]))
dev.off()
```

Figure 2B: Predicted nuclei, mitochondria and cytoplasm signals. 
```{r}
...
cent_df <- cent_df[order(cent_df$id),]
...
figname = paste0(OUTFIGS, filesep, "Fig2B_",thedate,".png")
...
p <- cent_labels %>% 
  filter(cell == 1)
text(p$x, p$y+4, labels=expression(bold(1)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 2)
text(p$x, p$y+4, labels=expression(bold(2)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 3)
text(p$x, p$y+4, labels=expression(bold(3)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 4)
text(p$x, p$y+4, labels=expression(bold(4)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 5)
text(p$x, p$y+4, labels=expression(bold(5)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 6)
text(p$x, p$y+4, labels=expression(bold(6)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 7)
text(p$x, p$y+4, labels=expression(bold(7)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 8)
text(p$x, p$y+4, labels=expression(bold(8)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 9)
text(p$x, p$y+4, labels=expression(bold(9)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 10)
text(p$x, p$y+4, labels=expression(bold(10)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 11)
text(p$x, p$y+4, labels=expression(bold(11)), col="green", cex=1)

p <- cent_labels %>% 
  filter(cell == 12)
text(p$x, p$y+4, labels=expression(bold(12)), col="green", cex=1)
...
figname = paste0(OUTFIGS, filesep, "Fig2B_crop.tif")
la=plotPNGorTIF(figname)

# figname = paste0(OUTFIGS, filesep, "Fig2B_b_",thedate,".png")
# png(figname)
# im <- bioimagetools::readTIF(paste0(OUTFIGS, filesep,"Fig2B_crop.tif"))
# im <- EBImage::resize(im, dim(im)[1]/1)
# plot(raster::as.raster(im[,,,1]))
# dev.off()
```
...
```{r}
figname=paste0(OUTFIGS, filesep, "Fig2C_snapshot.png")
if(!file.exists(figname)){
  coord__2 <- filter(coord__, id == 237)
  zlim=getZLIM(coord__2)
  # col=rainbow(length(unique(coord__$signal))*1.3)
  col=c("cyan", "yellow", "magenta")
  names(col)=as.character(unique(coord__2$signal))
  rgl::close3d()
  for(s in names(signals)){
    X=coord__2[coord__2$signal==s,]
    if(s=="nucleus.p"){
...
figname = paste0(OUTFIGS, filesep, "Fig2C_",thedate,".png")
png(figname)
im <- bioimagetools::readTIF(paste0(OUTFIGS, filesep,"Fig2C_snapshot.tif"))
im <- EBImage::resize(im, dim(im)[1]/1)
plot(raster::as.raster(im[,,,1]))
dev.off()
la=plotPNGorTIF(figname)
...
# fucci_export <- fucci_pseud %>% 
#   group_by(id) %>% 
#   summarise(pseudotime = mean(pseudotime))

# write.csv(fucci_pseud, file = paste0(OUTFIGS, filesep, FoF, "_fucci.csv"))
...
# sc <- scale_fill_gradientn(colours = pal(10), limits=c(0, 20.01))
sc <- scale_fill_distiller(palette = 'Greys')
for(i in unique(target_df$cell)) {
  ...
  a <- ggplot(p) +
    ...
    geom_tile(aes(x, y, alpha = fucci_ch00), fill = "green") +
  ...
  b <- ggplot(p) +
    ...
    labs(x = xlab) +
    geom_tile(aes(x, y, alpha = fucci_ch02), fill = "red") +
   scale_alpha(range = c(0, 0.3))
...
plot_ch00_cell247 <- ggplot(p) +
  geom_tile(aes(x, y, fill = z), alpha = 0.4) +
  scale_fill_distiller(guide = "none", palette = 'Greys') +
  theme_void() +
...
# Get single cell visualizations
# for(i in unique(coord_$id)) {
#    p <- coord_ %>% 
#      filter(id == i)
#    
#    print(ggplot(p) +
...
#      scale_fill_manual(values = c("nucleus.p" = "magenta", "mito.p" = "yellow", "cytoplasm.p" = "cyan")))
#    
#    ggsave(paste0(OUTFIGS, filesep, "cell_images", filesep, FoF, "_cell_", i, ".png"))
# }
...
png(figname, width = 1500,height = 1450)
eval(hm$call)
dev.off()
la=plotPNGorTIF(figname)
  ...
  for (r in 1:nrow) {
    for (c in 1:ncol) {
      if (panel_index <= length(plots[[fig]])) {
        ...
        xleft = if (fig == "Fig4" && panel == "C") 0 else (c - 1) * (panel_width + BORDER)  # Adjust xleft for Fig4C
        ybottom = total_height - r * (panel_height + BORDER)
        xright = if (fig == "Fig4" && panel == "C") total_width else xleft + panel_width  # Adjust xright for Fig4C
