...
figname=paste0(OUTFIGS, filesep, "Fig2B_snapshot.tif")
...
png(figname)
im <- bioimagetools::readTIF(paste0(OUTFIGS, filesep,"Fig2B_snapshot.tif"))
im <- EBImage::resize(im, dim(im)[1]/1)
plot(raster::as.raster(im[,,,1]))
dev.off()
```
...
# figname = paste0(OUTFIGS, filesep, "Fig2A_crop.tif")
# la=plotPNGorTIF(figname)

# figname = paste0(OUTFIGS, filesep, "Fig2A_b_",thedate,".png")
# png(figname)
...
for(coi in unique(target_df$id)){
  ...
  hull=Plot_ConcaveHull(a_$x, a_$y, a_$z, xlim = xyzlim[,"x"], ylim = xyzlim[,"y"], zlim = xyzlim[,"z"],level = 10, lcolor = "gray", alpha=0.25,add = F, phi = 30,theta = 140)
