...
Figure 2B,C: Nuclei, mitochondria and cytoplasm of 10 representative cells from region highlighted in (A). Every data point is color-coded according to its cell membership. 
```{r}
...
coi = coi[c(2,3,6,7,8,10,12)]
coord__=coord_[coord_$id %in% coi,]

panels=c("B","C")
names(panels) = coi[c(1,4)]
for(cell in names(panels)){
  figname = paste0(OUTFIGS, filesep, "Fig2",panels[cell],"_",thedate,".png")
  ...
  Plot_ConcaveHull(a_$x, a_$y, a_$z, xlim = xyzlim[,"x"], ylim = xyzlim[,"y"], zlim = xyzlim[,"z"],level = 10, lcolor = "purple", alpha=0.25,add = F, phi = 40,theta = 40)
