...
%FoV='FoFX001005_221018_brightfield';
FoV='FoFX002006_221018_brightfield';
...
b=table2array(csv(:,'hour'));
a=table2array(csv(:,'pseudotime'));
% a=[1,2,3,4,5,6];
% b=[7,8,3,4,5,6];
...
csv.Properties.VariableNames{size(csv, 2)} = 'hour_shifted';
writetable(csv,strrep(csvf,'.csv','_matlabOut.csv'))

%% Pearson before vs after shift
[r_b,p_b]=corr(table2array(csv(:,'pseudotime')),table2array(csv(:,'hour')))
[r,p]=corr(table2array(csv(:,'pseudotime')),table2array(csv(:,'hour_shifted')))
